...
def download_orders(request):
    ...
    while response_count < 1:
        ...
        if response.status_code == 200:
            ...
            save_to_gcp(res, api_method['download_orders'])
            response_count += 1
        else:
...
def result(request):
    download_installs(request)
    sleep(1)
    download_orders(request)
    sleep(1)
    download_costs(request)
    sleep(1)
    download_events(request)
    return 'ALL DONE !!!'